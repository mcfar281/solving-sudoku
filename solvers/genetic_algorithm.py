...
class CycleCrossover(object):
    ...
    def crossover_rows(self, row1, row2): 
        ...
        remaining = [num for num in range(1,Nd+1)]
        cycle = 0
...
class Sudoku(object):
    ...
    # Added by Mason McFarland to be compatible with numpy sudoku representation
    def load_array(self, numpy_array):
        self.given = Given(numpy_array)
        return
    ...
    def solve(self):
        Nc = 200  # Number of candidates (i.e. population size).
        Ne = int(0.1*Nc)  # Number of elites. MM - updated to .1 from Pathak and Kumar (2023)
        ...
        for generation in range(0, Ng):
            ...
            # Calculate new adaptive mutation rate. This is to stop too much mutation as the fitness approaches unity
            if Nm == 0:
